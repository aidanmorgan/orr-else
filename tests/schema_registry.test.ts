...
// ---------------------------------------------------------------------------
// AC3 (extended) — Legacy-version fail-closed policy
// ---------------------------------------------------------------------------

describe('AC3 (extended): legacy-version fail-closed behavior', () => {
  it('registration with legacyVersions but no migrationNote throws SchemaRegistryError', () => {
    const reg = new SchemaRegistry();
    expect(() =>
      reg.register(minimalEntry({
        id: 'harness.test.legacy.nomigrate',
        legacyVersions: ['1.0.0'],
        migrationNote: undefined
      }))
    ).toThrow(SchemaRegistryError);
    expect(() =>
      reg.register(minimalEntry({
        id: 'harness.test.legacy.nomigrate2',
        legacyVersions: ['1.0.0'],
        migrationNote: undefined
      }))
    ).toThrow(/migrationNote.*REQUIRED|REQUIRED.*migrationNote/i);
  });

  it('registration with legacyVersions AND migrationNote succeeds', () => {
    const reg = new SchemaRegistry();
    expect(() =>
      reg.register(minimalEntry({
        id: 'harness.test.legacy.ok',
        legacyVersions: ['1.0.0'],
        migrationNote: 'v1 had a flat structure; v2 wraps it in an object.'
      }))
    ).not.toThrow();
  });

  it('getValidator with a known-legacy (id, version) pair throws SchemaRegistryError', () => {
    const reg = new SchemaRegistry();
    reg.register(minimalEntry({
      id: 'harness.test.legacy.query',
      version: '2.0.0',
      legacyVersions: ['1.0.0'],
      migrationNote: 'Bumped major: removed deprecated `foo` field.'
    }));
    // Querying the current version is fine.
    expect(() => reg.getValidator('harness.test.legacy.query')).not.toThrow();
    // Querying a known-legacy version is rejected.
    expect(() =>
      reg.getValidator('harness.test.legacy.query', '1.0.0')
    ).toThrow(SchemaRegistryError);
    expect(() =>
      reg.getValidator('harness.test.legacy.query', '1.0.0')
    ).toThrow(/legacy/i);
  ...
  it('getValidator with a non-legacy version string does not throw', () => {
    const reg = new SchemaRegistry();
    reg.register(minimalEntry({
      id: 'harness.test.legacy.nonlegacy',
      version: '2.0.0',
      legacyVersions: ['1.0.0'],
      migrationNote: 'v1 deprecated.'
    }));
    // Querying with a version that is NOT in legacyVersions is fine.
    expect(() => reg.getValidator('harness.test.legacy.nonlegacy', '2.0.0')).not.toThrow();
    expect(() => reg.getValidator('harness.test.legacy.nonlegacy', '3.0.0')).not.toThrow();
  });

  it('registration with empty legacyVersions array and no migrationNote is fine', () => {
    const reg = new SchemaRegistry();
    // Empty array should not trigger the migrationNote requirement.
    expect(() =>
      reg.register(minimalEntry({
        id: 'harness.test.legacy.empty',
        legacyVersions: []
      }))
    ).not.toThrow();
  });
