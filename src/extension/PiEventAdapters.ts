...
// ── Pi toolCallId ↔ toolInvocationId typed adapter (dsm2.12) ─────────────────

/**
 * Typed adapter that maps a Pi-native toolCallId (present on tool_call /
 * tool_result Pi hook events) to the harness toolInvocationId (generated
 * per-invocation by wrapPluginTool / PiObservers).
 *
 * The mapping is explicit and typed: the Pi toolCallId is preserved as-is
 * alongside the harness toolInvocationId on every tool result event, so
 * consumers can correlate across Pi observer events, extension tool_call/
 * tool_result hooks, native Pi observed tools, cache hits, retries, failures,
 * and replay events.
 *
 * Both fields are optional: toolCallId may be absent (Pi does not always
 * provide one), and toolInvocationId is always present when generated by the
 * harness (but may be undefined for legacy events written before dsm2.12).
 *
 * Returns a typed record carrying both fields so callers can spread it directly
 * onto the event data object.
 */
export function mapPiToolCallIdToInvocationId(
  toolCallId: string | undefined,
  toolInvocationId: string
): { toolCallId: string | undefined; toolInvocationId: string } {
  return { toolCallId, toolInvocationId };
}
