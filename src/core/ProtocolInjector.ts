...
${mcpPolicy}
- **Tool Result Contract**: Every Orr Else tool returns its own minimal schema — there is no shared envelope with universal \`resultPreview\`, \`diagnosticPreview\`, \`outputPreview\`, \`outputArchive\`, or truncation flags. Complete raw output is archived to harness-managed tool-calls storage by the harness; the model-facing return is each tool's declared minimal schema. To obtain more detail, re-run the same tool with narrower arguments (e.g. a specific file path, projection name, or reduced scope) — do not try to access a generic raw-output path. See the tool-routing SKILL.md for each tool's schema fields, raw-output file/ref location, and rerun strategy.
