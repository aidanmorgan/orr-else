...
#### PHASE CONTRACT:
- **Mandatory Tasks**: Use \`${BuiltInToolName.GET_OUTSTANDING_TASKS}\` to query your deterministic completion checklist and \`${BuiltInToolName.TICK_ITEMS}\` to record completed checklist evidence (one or more items per call).
...
${mcpPolicy}
- **Tool Result Contract**: Every Orr Else tool returns its own minimal schema. There is no shared generic output-control envelope. Complete raw output is archived to harness-managed tool-output storage by the harness; the model-facing return is each tool's declared minimal schema. Tool-owned compaction fields (such as compactSummary, diagnosticFacts, structuredResult) may appear in project-tool results -- these are tool-specific, not harness-imposed. To obtain more detail, re-run the same tool with narrower arguments (e.g. a specific file path, projection name, or reduced scope) -- do not try to access a generic raw-output path. See the tool-routing SKILL.md for each tool's schema fields, raw-output file/ref location, and rerun strategy.
